...
bool SpiNorFlash::WriteInProgress() { return (ReadStatusRegister() & 0x01u) == 0x01u; }

bool SpiNorFlash::WriteEnabled() { return (ReadStatusRegister() & 0x02u) == 0x02u; }
...
void SpiNorFlash::Read(uint32_t address, uint8_t* buffer, size_t size) {
    static constexpr uint8_t cmdSize = 4;
    uint8_t cmd[cmdSize] = {static_cast<uint8_t>(Commands::Read), static_cast<uint8_t>(address >> 16U), static_cast<uint8_t>(address >> 8U),
...
void SpiNorFlash::SectorErase(uint32_t sectorAddress) {
    static constexpr uint8_t cmdSize = 4;
    uint8_t cmd[cmdSize] = {static_cast<uint8_t>(Commands::SectorErase), static_cast<uint8_t>(sectorAddress >> 16U),
                            static_cast<uint8_t>(sectorAddress >> 8U), static_cast<uint8_t>(sectorAddress)};
...
bool SpiNorFlash::ProgramFailed() { return (ReadSecurityRegister() & 0x20u) == 0x20u; }

bool SpiNorFlash::EraseFailed() { return (ReadSecurityRegister() & 0x40u) == 0x40u; }
...
void SpiNorFlash::Write(uint32_t address, const uint8_t* buffer, size_t size) {
    ...
    while (len > 0) {
        ...
        uint8_t cmd[cmdSize] = {static_cast<uint8_t>(Commands::PageProgram), static_cast<uint8_t>(addr >> 16U),
                                static_cast<uint8_t>(addr >> 8U), static_cast<uint8_t>(addr)};
