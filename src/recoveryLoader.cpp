...
void RefreshWatchdog() { NRF_WDT->RR[0] = WDT_RR_RR_Reload; }
...
void DisplayProgressBar(uint8_t percent, uint16_t color) {
    ...
    for (int i = 0; i < barHeight; i++) {
        ...
        lcd.DrawBuffer(0, displayWidth - barHeight + i, barWidth, 1, reinterpret_cast<const uint8_t*>(displayBuffer),
...
int main(void) {
    ...
    if (pdPASS != xTaskCreate(Process, "MAIN", 512, nullptr, 0, &taskHandle)) APP_ERROR_HANDLER(NRF_ERROR_NO_MEM);
