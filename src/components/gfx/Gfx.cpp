...
void Gfx::pixel_draw(uint8_t x, uint8_t y, uint16_t color) { lcd.DrawPixel(x, y, color); }
    ...
    if (state.action == Action::FillRectangle) {
        *data = reinterpret_cast<uint8_t*>(buffer);
        ...
        for (uint16_t j = 0; j < bytes_in_line; j++) {
            for (uint8_t k = 0; k < 8; k++) {
                if ((1 << (7 - k)) &
                    state.font->data[state.font->charInfo[char_idx].offset + ((state.currentIteration + 1) * bytes_in_line) + j]) {
...
void Gfx::WaitTransferFinished() const { ulTaskNotifyTake(pdTRUE, 500); }
...
void Gfx::SetScrollStartLine(uint16_t line) { lcd.VerticalScrollStartAddress(line); }
