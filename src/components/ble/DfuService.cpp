...
int DfuService::WritePacketHandler(uint16_t connectionHandle, os_mbuf* om) {
    switch (state) {
        ...
        case States::Init: {
            ...
            NRF_LOG_INFO("[DFU] -> Init data received : deviceType = %d, deviceRevision = %d, applicationVersion = %d, nb SD = %d, First "
                         "SD = %d, CRC = %u",
                         deviceType,
                         deviceRevision,
                         applicationVersion,
        ...
        case States::Data: {
            ...
            if ((nbPacketReceived % nbPacketsToNotify) == 0 && bytesReceived != applicationSize) {
                uint8_t data[5] {static_cast<uint8_t>(Opcodes::PacketReceiptNotification),
                                 static_cast<uint8_t>(bytesReceived & 0x000000FFu), static_cast<uint8_t>(bytesReceived >> 8u),
                                 static_cast<uint8_t>(bytesReceived >> 16u), static_cast<uint8_t>(bytesReceived >> 24u)};
                NRF_LOG_INFO("[DFU] -> Send packet notification: %d bytes received", bytesReceived);
                notificationManager.Send(connectionHandle, controlPointCharacteristicHandle, data, 5);
            }
            if (dfuImage.IsComplete()) {
                uint8_t data[3] {static_cast<uint8_t>(Opcodes::Response), static_cast<uint8_t>(Opcodes::ReceiveFirmwareImage),
...
int DfuService::ControlPointHandler(uint16_t connectionHandle, os_mbuf* om) {
    ...
    switch (opcode) {
        ...
        case Opcodes::InitDFUParameters: {
            ...
            if (isInitComplete) {
                uint8_t data[3] {static_cast<uint8_t>(Opcodes::Response), static_cast<uint8_t>(Opcodes::InitDFUParameters),
        ...
        case Opcodes::ValidateFirmware: {
            ...
            if (dfuImage.Validate()) {
                ...
                uint8_t data[3] {static_cast<uint8_t>(Opcodes::Response), static_cast<uint8_t>(Opcodes::ValidateFirmware),
                                 static_cast<uint8_t>(ErrorCodes::NoError)};
                notificationManager.AsyncSend(connectionHandle, controlPointCharacteristicHandle, data, 3);
            } else {
...
void DfuService::DfuImage::Append(uint8_t* data, size_t size) {
    ...
    if (bufferWriteIndex > 0 && totalWriteIndex + bufferWriteIndex == totalSize) {
        spiNorFlash.Write(writeOffset + totalWriteIndex, tempBuffer, bufferWriteIndex);
        totalWriteIndex += bufferWriteIndex;
        if (totalSize < maxSize) WriteMagicNumber();
    }
}
