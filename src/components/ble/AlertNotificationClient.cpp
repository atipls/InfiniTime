...
namespace {
    ...
    int OnAlertNotificationDescriptorDiscoveryEventCallback(uint16_t conn_handle, const struct ble_gatt_error* error,
...
bool AlertNotificationClient::OnDiscoveryEvent(uint16_t connectionHandle, const ble_gatt_error* error, const ble_gatt_svc* service) {
    if (service == nullptr && error->status == BLE_HS_EDONE) {
        if (isDiscovered) {
            ...
            ble_gattc_disc_all_chrs(connectionHandle, ansStartHandle, ansEndHandle, OnAlertNotificationCharacteristicDiscoveredCallback,
                                    this);
        } else {
...
int AlertNotificationClient::OnDescriptorDiscoveryEventCallback(uint16_t connectionHandle, const ble_gatt_error* error,
    ...
    if (error->status == 0) {
        ...
        if (!isDescriptorFound) onServiceDiscovered(connectionHandle);
    }
    return 0;
}
