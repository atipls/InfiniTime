...
Notifications::Notifications(DisplayApp* app, Pinetime::Controllers::NotificationManager& notificationManager,
                             ...
                             Pinetime::Controllers::MotorController& motorController, System::SystemTask& systemTask, Modes mode)
  : app {app},
    ...
    if (notification.valid) {
        ...
        currentItem = std::make_unique<NotificationItem>(notification.Title(), notification.Message(), 1, notification.category,
                                                         notificationManager.NbNotifications(), alertNotificationService, motorController);
        validDisplay = true;
    } else {
...
void Notifications::Refresh() {
    if (mode == Modes::Preview && timeoutLine != nullptr) {
        ...
        if (validDisplay) {
            ...
            currentItem =
              std::make_unique<NotificationItem>(notification.Title(), notification.Message(), currentIdx + 1, notification.category,
                                                 notificationManager.NbNotifications(), alertNotificationService, motorController);
        } else {
            currentItem = std::make_unique<NotificationItem>(alertNotificationService, motorController);
        }
...
bool Notifications::OnTouchEvent(Pinetime::Applications::TouchEvents event) {
    ...
    switch (event) {
        ...
        case Pinetime::Applications::TouchEvents::SwipeDown: {
            ...
            currentItem.reset(nullptr);
            app->SetFullRefresh(DisplayApp::FullRefreshDirections::Down);
            currentItem = std::make_unique<NotificationItem>(previousNotification.Title(), previousNotification.Message(), currentIdx + 1,
                                                             previousNotification.category, notificationManager.NbNotifications(),
                                                             alertNotificationService, motorController);
        }
            return true;
        case Pinetime::Applications::TouchEvents::SwipeUp: {
            ...
            currentItem.reset(nullptr);
            app->SetFullRefresh(DisplayApp::FullRefreshDirections::Up);
            currentItem = std::make_unique<NotificationItem>(nextNotification.Title(), nextNotification.Message(), currentIdx + 1,
                                                             nextNotification.category, notificationManager.NbNotifications(),
                                                             alertNotificationService, motorController);
        }
            return true;
...
Notifications::NotificationItem::NotificationItem(const char* title, const char* msg, uint8_t notifNr,
                                                  Controllers::NotificationManager::Categories category, uint8_t notifNb,
                                                  Pinetime::Controllers::AlertNotificationService& alertNotificationService,
                                                  Pinetime::Controllers::MotorController& motorController)
  : alertNotificationService {alertNotificationService}, motorController {motorController} {
    ...
    switch (category) {
        default: lv_label_set_text(alert_subject, msg); break;
...
Notifications::NotificationItem::~NotificationItem() { lv_obj_clean(lv_scr_act()); }
